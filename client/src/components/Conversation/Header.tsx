...
function Header({ socketRef }: HeaderProps) {
    ...
    const currentUser: string = useAppSelector((state) =>
    ...
    const channel: ChannelModel =
        useAppSelector((state) => selectDisplayedChannel(state)) ||
        emptyChannel;
    const isPrivateConv: boolean =
        channel.members?.length === 1 && channel.type === "privateConv"
            ? true
            : false;

...
export class MuteSchedulerService {
	...
	for (const user of usersWithMutedChannels) {
		const channelsToUnmuteUser = user.MutedInChannels.filter(
			(channel) => channel.muted.some((mutedUser) => mutedUser.login === user.login),
		);
        ...
        this.logger.debug(
          `Unmuted user ${user.login} in channels: ${channelsToUnmuteUser.map((channel) => channel.name).join(', ')}.`,
        );
      }
    }
  }
